...
class PriceTracker {
  ...
  async updatePrices() {
    ...
    try {
      const stocks = this.db.prepare('SELECT id, symbol, current_price FROM stocks WHERE is_active = TRUE LIMIT 10').all();
      ...
      for (const data of priceData) {
        ...
        try {
          // Update database
          this.db.prepare('UPDATE stocks SET current_price = ? WHERE id = ?').run(data.price, stock.id);
